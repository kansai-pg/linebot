...
def long_term_non_responders():
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cur:
                print(day, hour)
                cur.execute("select user_id, room_name, TO_CHAR(lastpush,'yyyy/mm/dd'), ID from mainid WHERE LASTPUSH <= ADD_MONTHS(:0, -3) and status != 6", (day,))
                for col in cur:
                    #jsonで送信内容を書く
                    req_data = {
                        "to": col[0],
